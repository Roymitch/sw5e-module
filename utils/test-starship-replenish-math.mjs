...
test("normalize signed: missing/invalid → 0; preserve +/−/0; trunc decimals", () => {
	assert.equal(normalizeStarshipSignedInt(undefined), 0);
	assert.equal(normalizeStarshipSignedInt(null), 0);
	assert.equal(normalizeStarshipSignedInt(""), 0);
	assert.equal(normalizeStarshipSignedInt("  "), 0);
	assert.equal(normalizeStarshipSignedInt(NaN), 0);
	assert.equal(normalizeStarshipSignedInt(Infinity), 0);
	assert.equal(normalizeStarshipSignedInt(-Infinity), 0);
	assert.equal(normalizeStarshipSignedInt("refuel"), 0);
	assert.equal(normalizeStarshipSignedInt("restock"), 0);
	assert.equal(normalizeStarshipSignedInt(40), 40);
	assert.equal(normalizeStarshipSignedInt(-20), -20);
	assert.equal(normalizeStarshipSignedInt(0), 0);
	assert.equal(normalizeStarshipSignedInt("7"), 7);
	assert.equal(normalizeStarshipSignedInt("-3"), -3);
	assert.equal(normalizeStarshipSignedInt(4.9), 4);
	assert.equal(normalizeStarshipSignedInt(-4.9), -4);
});

test("food capacity: size baseline without override", () => {
	const r = resolveStarshipFoodCapacity(120, 999, false, 0);
	assert.equal(r.effectiveCap, 120);
	assert.equal(r.selectedBase, 120);
	assert.equal(r.overrideActive, false);
	assert.equal(r.outsideRaw, false);
});

test("food capacity: Size 120 + mod 40 = 160; Size 120 + mod -20 = 100", () => {
	assert.equal(resolveStarshipFoodCapacity(120, 0, false, 40).effectiveCapacity, 160);
	assert.equal(resolveStarshipFoodCapacity(120, 0, false, -20).effectiveCapacity, 100);
});

test("food capacity: Size 10 + mod -20 floors to 0 with unclamped -10", () => {
	const r = resolveStarshipFoodCapacity(10, 0, false, -20);
	assert.equal(r.unclampedEffectiveCapacity, -10);
	assert.equal(r.effectiveCapacity, 0);
});

test("food capacity: Custom 200 + mod -50 = 150; same mod in Size mode", () => {
	const custom = resolveStarshipFoodCapacity(120, 200, true, -50);
	assert.equal(custom.selectedBase, 200);
	assert.equal(custom.effectiveCapacity, 150);
	const size = resolveStarshipFoodCapacity(120, 200, false, -50);
	assert.equal(size.selectedBase, 120);
	assert.equal(size.effectiveCapacity, 70);
});

test("food capacity: actor override outsideRaw by custom base only", () => {
	const r = resolveStarshipFoodCapacity(120, 150, true, 40);
	assert.equal(r.effectiveCap, 190);
	assert.equal(r.outsideRaw, true);
	assert.equal(r.tinyPositiveCustom, false);
	const equalBase = resolveStarshipFoodCapacity(120, 120, true, 40);
	assert.equal(equalBase.outsideRaw, false);
	assert.equal(equalBase.effectiveCapacity, 160);
	const sizeWithMod = resolveStarshipFoodCapacity(120, 999, false, 40);
	assert.equal(sizeWithMod.outsideRaw, false);
});

test("food capacity: Tiny/zero baseline and positive custom", () => {
	const baseline = resolveStarshipFoodCapacity(0, 0, false, 0);
	assert.equal(baseline.effectiveCap, 0);
	const override = resolveStarshipFoodCapacity(0, 5, true, 0);
	assert.equal(override.effectiveCap, 5);
	assert.equal(override.tinyPositiveCustom, true);
	assert.equal(override.tinyPositiveOverride, true);
	assert.equal(override.outsideRaw, true);
	const tinyMod = resolveStarshipFoodCapacity(0, 0, false, 5);
	assert.equal(tinyMod.effectiveCapacity, 5);
	assert.equal(tinyMod.outsideRaw, false);
});

test("food capacity: size baseline change with/without override", () => {
	const noOverrideSmall = resolveStarshipFoodCapacity(10, 0, false, 0);
	const noOverrideMedium = resolveStarshipFoodCapacity(120, 0, false, 0);
	assert.equal(noOverrideSmall.effectiveCap, 10);
	assert.equal(noOverrideMedium.effectiveCap, 120);

	const withOverride = resolveStarshipFoodCapacity(120, 50, true, 0);
	const sizeChanged = resolveStarshipFoodCapacity(240000, 50, true, 0);
...
test("food capacity: large RAW values stay stable integers", () => {
	const garg = resolveStarshipFoodCapacity(576000000, 0, false, 0);
...
test("food capacity: missing size baseline → 0; unsafe addition flagged", () => {
	assert.equal(resolveStarshipFoodCapacity(undefined, 10, false, 0).effectiveCap, 0);
	assert.equal(resolveStarshipFoodCapacity(null, 10, false, 0).effectiveCap, 0);
	const unsafe = resolveStarshipFoodCapacity(Number.MAX_SAFE_INTEGER, 0, false, 2);
	assert.equal(unsafe.safeInteger, false);
	assert.equal(unsafe.effectiveCapacity, Math.max(0, Number.MAX_SAFE_INTEGER + 2));
});

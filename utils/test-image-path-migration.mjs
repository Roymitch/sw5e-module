...
check("V14 system.changes-only effect remaps icon and preserves the change collection", () => {
	const effect = {
		name: "Auto-Thrusters",
		icon: "modules/sw5e/icons/fx.webp",
		system: { changes: [{ key: "system.abilities.dex.bonuses.save", mode: 2, value: "+1" }] }
	};
	const beforeChanges = structuredClone(effect.system.changes);
	const update = applyImagePathMigration(effect, {}, { getModulePath });
	assert.equal(update.icon, "modules/sw5e-module/icons/fx.webp");
	assert.equal("system.changes" in update, false);
	assert.deepEqual(effect.system.changes, beforeChanges);
});

check("legacy top-level changes effect remaps icon", () => {
	const effect = {
		name: "Legacy AE",
		icon: "modules/sw5e/icons/fx.webp",
		changes: [{ key: "system.abilities.dex.bonuses.save", mode: 2, value: "+1" }]
	};
	const update = applyImagePathMigration(effect, {}, { getModulePath });
	assert.equal(update.icon, "modules/sw5e-module/icons/fx.webp");
	assert.equal("changes" in update, false);
});

check("valid portrait and prototype token stay exact equals after migration", () => {
	const actor = {
		type: "character",
		img: "worlds/demo/portraits/pc.png",
		prototypeToken: { texture: { src: "worlds/demo/tokens/pc.png" } }
	};
	const beforeImg = actor.img;
	const beforeTok = actor.prototypeToken.texture.src;
	const update = migrate(actor);
	assert.deepEqual(update, {});
	assert.equal(actor.img, beforeImg);
	assert.equal(actor.prototypeToken.texture.src, beforeTok);
	assert.equal(beforeImg, "worlds/demo/portraits/pc.png");
	assert.equal(beforeTok, "worlds/demo/tokens/pc.png");
});

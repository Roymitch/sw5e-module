...
### Fixed
- **Starship hidden crew membership (stowaway):** GMs can Hide/Reveal crew members on the Core Flight Manifest so non-GM viewers (including Starship Owners) do not see hidden members. Hidden rows remain visible only to GMs at reduced opacity; Hide/Reveal refreshes the open sheet immediately; the Flight Manifest count uses the viewer’s filtered roster. Alt-drop into hidden membership is not included yet.
